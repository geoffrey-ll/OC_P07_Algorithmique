...
def perf_of_file(path_file_actions, max_line, who):
    time_start = t_time()
    if who == "bruteforce":
        line_num = main_bruteforce(path_file_actions, max_line)
    if who == "optimized":
        line_num = main_optimized(path_file_actions, max_line)
    time_end = t_time()
...
def main_analyzer(path_file_actions, who):
    ...
    while max_line >= 2 or max_line == -1:
        runtime, line_num = perf_of_file(path_file_actions, max_line, who)
...
def graph(number_actions, runtime, who):
    ...
    color1 = "red"
    line1 = ax1.scatter(number_actions, runtime, label="Durée", color=color1)
